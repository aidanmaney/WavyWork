...
let init = (app) => {
    ...
    app.add_task = () => {
        ...
        axios
            ...
            .then(function (response) {
                ...
                app.apply_date_data_to_tasks(
                    app.enumerate(app.vue.all_user_tasks)
                );
    ...
    app.spawn_next_reflection_modal = () => {
        if (app.vue.active_tasks.length > 0) {
            app.vue.reflection_modal_tasks.push(
                app.vue.active_tasks[app.vue.active_tasks.length - 1]
            );
            ...
            document.getElementById("journal_modal_title").innerHTML =
    ...
    app.submit_task_reflection = (task_id) => {
        axios
            .post(submit_task_reflection_url, {
                task_id: task_id,
    ...
    app.set_group_member_status = (a) => {
        a.map((e) => {
            Vue.set(e, "is_group_member", false);
        });
        return a;
    };
    ...
    app.search = () => {
        ...
        for (u of app.vue.all_users) {
            if (
                u.first_name
                    .toLowerCase()
                    .startsWith(app.vue.search_query.toLowerCase())
            ) {
    ...
    app.check_if_reflections_available = () => {
        ...
        axios.get(check_for_submitted_reflections_url).then((res) => {
            if (
                (time >= 18 || time < 8) &&
                res.data.todays_reflections.length <= 0
            ) {
    ...
    app.submit_journal_entry = () => {
        axios
            .post(submit_journal_entry_url, {
                entry: app.vue.reflection_journal,
            })
            .then(() => {
    ...
    app.toggle_subtask_complete = (t_idx) => {
        axios
            .post(toggle_subtask_complete_url, {
                subtask_id: app.vue.task_view_subtasks[t_idx].id,
            })
            .then(() => {
                app.vue.task_view_subtasks[t_idx].is_complete =
                    !app.vue.task_view_subtasks[t_idx].is_complete;
            });
    };
    ...
    app.apply_date_data_to_tasks = (all_tasks) => {
        ...
        for (task of all_tasks) {
            ...
            date_info_for_tasks.push({
                ...
                start_relative_to_today:
                    start_relative_to_today >= 0 ? start_relative_to_today : 0,
        ...
        let k = 0;
        all_tasks.map((e) => {
            Vue.set(e, "date_data", date_info_for_tasks[k++]);
        });
        return all_tasks;
    };
    ...
    app.get_all_users_tasks = () => {
        axios.get(get_all_users_tasks_url).then((res) => {
            ...
            app.vue.all_user_tasks = app.apply_date_data_to_tasks(
                app.enumerate(res.data.all_users_tasks)
            );
        });
    };
    ...
    app.open_expanded_task_view = (t_idx) => {
        ...
        axios
            .get(get_task_subtasks_url, {
                params: { task_id: expanded_task.id },
            })
            .then((res) => {
        ...
        if (expanded_task.is_group) {
            axios
                .get(get_group_members_url, {
                    params: { group_id: expanded_task.group_id },
                })
                .then((res) => {
        ...
        document
            .getElementById("expanded_task_view_modal")
